/**
 * GraphQL Client for Admin Dashboard
 * 
...
export class GraphQLClient {
  ...
  setAuthToken(token: string | null) {
    ...
    if (typeof window !== "undefined") {
      if (token) {
        localStorage.setItem("admin_graphql_auth_token", token);
      } else {
        localStorage.removeItem("admin_graphql_auth_token");
      }
    }
  }
  ...
  getAuthToken(): string | null {
    if (typeof window !== "undefined") {
      return localStorage.getItem("admin_graphql_auth_token") || this.authToken;
    }

      ...
      <div class="flex items-center gap-8">
        ...
        <nav class="hidden md:flex items-center gap-6">
          {#each navLinks as link}
            <a
              href={link.href}
              class="text-sm font-medium transition-colors hover:text-brand-500 {$page.url.pathname.startsWith(link.href) ? 'text-brand-600 dark:text-brand-400' : 'text-gray-600 dark:text-gray-400'}"
            >
              {link.label}
            </a>
          {/each}

    ...
    <div className="container mx-auto px-4 py-16 max-w-7xl">
      ...
      <nav className="mb-8 text-sm text-muted-foreground" aria-label="Breadcrumb">
        <ol className="flex items-center gap-2">
          ...
          <li>/</li>
          <li>
            <Link href={`/article/${category}`} className="hover:text-foreground">
              {parentCategory.name}
            </Link>
          </li>
          <li>/</li>
      ...
      <div className="mb-12">
        ...
        {subCategoryData.count !== undefined && (
          <p className="text-sm text-muted-foreground">
            {subCategoryData.count} {subCategoryData.count === 1 ? 'article' : 'articles'}
          </p>
        )}
      </div>
      ...
      <div>
        {posts.length > 0 ? (
          <div className="grid grid-cols-1 gap-8 md:grid-cols-2 lg:grid-cols-3">
            {posts.map((post) => {
              ...
              const postCategoryPath = postParentCategory && primaryCategory
                ? `/article/${postParentCategory.slug}/${primaryCategory.slug}`
                : primaryCategory
          ...
          <p className="text-muted-foreground">
            No articles in this subcategory yet. Check back soon!
          </p>
        )}
      </div>
    </div>

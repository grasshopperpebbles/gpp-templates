 ...
 *
 *   try {
 *     await limiter.check(10, identifier) // 10 requests per interval
 *   } catch {
...
export function getClientIdentifier(request: Request): string {
  // Try to get from headers (set by middleware or auth)
...
export const RateLimitPresets = {
   ...
   * Use for: Reviews, contact forms, user-generated content
   */
  standard: () => rateLimit({ interval: 60 * 1000, uniqueTokenPerInterval: 500 }),

  /**
   * Relaxed: 60 requests per minute (1 per second)
   * Use for: Product searches, browsing, general API calls
   */
  relaxed: () => rateLimit({ interval: 60 * 1000, uniqueTokenPerInterval: 500 }),

  /**
   * Payment: 3 requests per minute
   * Use for: Payment processing, critical financial operations
   */

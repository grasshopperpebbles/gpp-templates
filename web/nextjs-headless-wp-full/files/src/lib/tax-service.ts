...
/**
 * Check if tax calculation is configured
 */
...
export async function calculateTax(
  ...
  // Return 0 tax if Stripe is not configured
  if (!stripe) {
    console.warn('Stripe not configured. Set STRIPE_SECRET_KEY environment variable for tax calculation.')
    return {
      taxAmount: 0,
      taxAmountCents: 0,
      taxRate: 0,
